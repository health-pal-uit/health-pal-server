...
export class PostsMediasController {
  ...
  async create(
    ...
    return await this.postsMediasService.create(createPostsMediaDto, fileBuffer, fileName);
  }
  ...
  async update(
    ...
    const fileBuffer = file?.buffer;
    const fileName = file?.originalname;
    return await this.postsMediasService.update(id, updatePostsMediaDto, fileBuffer, fileName);
  }

  @Delete(':id')
  async remove(@Param('id') id: string) {
    return await this.postsMediasService.remove(id);
  }
}
